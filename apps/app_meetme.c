...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 379885 $")
		...
		<syntax>
			...
			<parameter name="options">
				<optionlist>
					...
					<option name="u" hasparams="optional">
						<para>Set attendee user value. (May be used to identify the caller).</para>
						<argument name="uservalue" required="true" />
					</option>
...
#define CONFFLAG_USERVALUE (1ULL << 33)
enum {
...
AST_APP_OPTIONS(meetme_opts, BEGIN_OPTIONS
	...
	AST_APP_OPTION_ARG('u', CONFFLAG_USERVALUE, OPT_ARG_USERVALUE ),
...
static const char * const slatogglebarge_app = "SLAToggleBarge";
...
struct ast_conference {
	...
	const char *post_process;		/*!< Command to run against recorded conference on Exit */
...
struct sla_trunk {
	...
	//YZhou
	/*! Channel that put the trunk on hold, should be the last active station on the trunk */ 
	struct ast_channel *chan_init_hold;
	/*! Name of the channel that dial from the trunk */ 
	char   dial_chan_name[AST_CHANNEL_NAME];
	/*! The call is transferred from another trunk */ 
	struct sla_trunk   *transfer_trunk;
	struct ast_channel *transfer_chan;
	//End
	unsigned int ring_timeout;
	...
	//YZhou
 	/*! If set to 1, no station will be able to join an active call with
         *  this trunk for the current call. It should always reset to 0 after call ends */
	unsigned int barge_disabled_per_call:1;
...
enum sla_event_type {
	...
	SLA_EVENT_UNHOLD,
	/*! Enable/Disable per call barge in */
	SLA_EVENT_TOGGLE_PER_CALL_BARGE,
	/*! Hold by Savant device */
        SLA_EVENT_HOLD_SAVANT,
	//End
};
...
static int releaseSLATrunk(struct ast_channel *chan);
...
static int conf_free(struct ast_conference *conf)
{
	...
	if (conf->recording == MEETME_RECORD_ACTIVE) {
		...
		if (conf->post_process) {
			if (conf->recordingfilename) {
				strcat(fullstring, conf->post_process);
				ast_verb(3, "Executing %s\n", fullstring);
				ast_safe_system(fullstring);
			}
		}
	}
	...
	struct ast_conference *conf)
{
	...
	ast_log(LOG_NOTICE, "SLA queue event conf chan %s\n", chan->name);
	...
	AST_RWLIST_TRAVERSE(&sla_stations, station, entry) {
        	AST_LIST_TRAVERSE(&station->trunks, trunk_ref, entry) {
			if(trunk_ref->trunk && trunk_ref->trunk->name)
				ast_log(LOG_NOTICE, "trunk_ref->trunk->name %s\n", trunk_ref->trunk->name);
			if(trunk_ref->chan && trunk_ref->chan->name)
				ast_log(LOG_NOTICE, "trunk_ref->chan %s\n", trunk_ref->chan->name);
...
static void send_talking_event(struct ast_channel *chan, struct ast_conference *conf, struct ast_conf_user *user, int talking)
{
	ast_manager_event(chan, EVENT_FLAG_CALL, "MeetmeTalking",
	      ...
	      "Uservalue: %s\r\n"
	      "Status: %s\r\n",
	      chan->name, chan->uniqueid, conf->confno, user->user_no, user->usrvalue, talking ? "on" : "off");
}
...
static int conf_run(struct ast_channel *chan, struct ast_conference *conf, struct ast_flags64 *confflags, char *optargs[])
{
	...
	ast_verb(4, "Checking for Uservalue\n");
	if (ast_test_flag64(confflags,CONFFLAG_USERVALUE)) {
		opt_usrvalue = optargs[OPT_ARG_USERVALUE];
		ast_verb(4, "Found Uservalue: %s\n", opt_usrvalue);
	}
	...
	if (ast_test_flag64(confflags, CONFFLAG_RECORDCONF)) {
		if (!conf->recordingfilename) {
			...
			/*Set the comand to run when finished recording */
			conf->post_process = pbx_builtin_getvar_helper(chan, "MEETME_RECORDINGEXEC");
			if (!conf->recordingformat) {
				conf->recordingformat = ast_strdup("wav");
			}
			ast_verb(4, "Starting recording of MeetMe Conference %s into file %s.%s.\n",
	...
	user->talking = -1;
	snprintf(user->usrvalue, sizeof(user->usrvalue), "%s", opt_usrvalue);
 ...
 dahdiretry:
	...
	//YZhou
	//The first user in the SLA meetme is the SLA trunk channel
	int trunk_usrno = 1;
	struct ast_conf_user *trunk_user;
	trunk_user = ao2_find(conf->usercontainer, &trunk_usrno, 0);
	if(trunk_user)
	{
		ast_log(LOG_NOTICE, "SLA trunk channel: %s\n", trunk_user->chan->name);
		if(user->user_no > 1)
		{
			chan->sla_trunk_channel = trunk_user->chan;
		}
	}
	//End
	if (!sent_event) {
			...
			*/
			...
			if ((user->adminflags & (ADMINFLAG_MUTED | ADMINFLAG_SELFMUTED)) && (dahdic.confmode & DAHDI_CONF_TALKER)) {
				...
				ast_manager_event(chan, EVENT_FLAG_CALL, "MeetmeMute",
					...
					"Uservalue: %s\r\n"
					"Status: on\r\n",
					chan->name, chan->uniqueid, conf->confno, user->user_no, user->usrvalue);
			}
			...
			if (c) {
				 ...
				 */
					...
					case AST_CONTROL_HOLD:
						...
						break;
					//YZhou
					case AST_CONTROL_HOLD_SAVANT:
                                                sla_queue_event_conf(SLA_EVENT_HOLD_SAVANT, chan, conf);
                                                break;
					case AST_CONTROL_UNHOLD:
						ast_log(LOG_NOTICE, "SLA queue UNHOLD\n");
						sla_queue_event_conf(SLA_EVENT_UNHOLD, chan, conf);
						break;
					case AST_CONTROL_TOGGLE_PER_CALL_BARGE:
						ast_debug(2, "SLA queue toggle per call barge control\n");
						sla_queue_event_conf(SLA_EVENT_TOGGLE_PER_CALL_BARGE, chan, conf);
						break;
					//End
					default:
						break;
				...
				} else if(f->frametype == AST_FRAME_INFO) {
					//YZhou 
					ast_debug(1, "Got INFO frame on channel %s\n", chan->name);
					conf_queue_dtmf(conf, user, f);
				} else {
					ast_debug(1, 
	...
	const char *name)
{
	...
	AST_LIST_TRAVERSE(&station->trunks, trunk_ref, entry) {
		if (strcasecmp(trunk_ref->trunk->name, name))
			continue;
		//YZhou
		//Check barge_disabled_per_call, too
		if ( ((trunk_ref->trunk->barge_disabled || trunk_ref->trunk->barge_disabled_per_call || trunk_ref->trunk->transfer_chan) 
...
static struct sla_ringing_station *sla_create_ringing_station(struct sla_station *station)
{
	...
	//YZhou
	ringing_station->answered_elsewhere = 0;
...
//YZhou
//trunk was "const struct sla_trunk *"
static void sla_change_trunk_state(struct sla_trunk *trunk, enum sla_trunk_state state, 
	enum sla_which_trunk_refs inactive_only, const struct sla_trunk_ref *exclude)
{
	...
	switch (state) {
	case SLA_TRUNK_STATE_IDLE:
		trunk->barge_disabled_per_call = 0;
		if(trunk->transfer_trunk)
		{
			ast_log(LOG_NOTICE, "Transfer trunk %s exists for trunk %s, change transfer trunk to idle\n", trunk->transfer_trunk->name, trunk->name);
			sla_change_trunk_state(trunk->transfer_trunk, state, inactive_only, exclude);
			trunk->transfer_trunk = NULL;
		}
		break;
	default:
		break;
	...
	AST_LIST_TRAVERSE(&sla_stations, station, entry) {
		AST_LIST_TRAVERSE(&station->trunks, trunk_ref, entry) {
			//YZhou
			ast_debug(1, "change trunk state SLA:%s_%s\n", station->name, trunk->name);
			if (trunk_ref->trunk != trunk || (inactive_only ? trunk_ref->chan : 0)
				|| trunk_ref == exclude)
			{
				ast_debug(1, "Skip SLA:%s_%s\n", station->name, trunk->name);
				continue;
			}
...
static int releaseSLATrunk(struct ast_channel *chan)
{
        ...
        {
                AST_RWLIST_RDLOCK(&sla_trunks);
                AST_LIST_TRAVERSE(&sla_trunks, tmp_trunk, entry) {
                        if(tmp_trunk && tmp_trunk->transfer_chan && tmp_trunk->transfer_chan->name)
                        {
                                if (!strcmp(tmp_trunk->transfer_chan->name, chan->name))
                                {
                                        ast_log(LOG_NOTICE, "Channel %s was bound to trunk %s\n", chan->name, tmp_trunk->name);
                                        sla_change_trunk_state(tmp_trunk, SLA_TRUNK_STATE_IDLE, ALL_TRUNK_REFS, NULL);
                                }
                        }
                }
                AST_RWLIST_UNLOCK(&sla_trunks);
...
static void *run_station(void *data)
{
	...
	//YZhou
	ast_debug(1, "Enter function run_station, active stations %d\n", trunk_ref->trunk->active_stations);
	struct ast_channel *original_trunk_ref_chan = trunk_ref->chan;
	...
	if(trunk_ref->chan && trunk_ref->chan->name)
	{
		ast_log(LOG_NOTICE, "Trunk ref channel before conf_run %s\n", trunk_ref->chan->name);
	}
	...
	//YZhou
	//See if the channel is masqueraded duo to trnasfer
	int    masqueraded = 0;
	struct ast_channel *target = NULL;
	if(trunk_ref && trunk_ref->chan)
	{
		masqueraded = trunk_ref->chan->masqueraded;
		if(trunk_ref->chan && trunk_ref->chan->name)
		{
			ast_log(LOG_NOTICE, "Trunk ref channel after conf_run %s masqueraded %d\n", trunk_ref->chan->name, trunk_ref->chan->masqueraded);
		}
		if(trunk_ref->chan->target)
		{
			if(trunk_ref->chan->target->name)
			{
				ast_log(LOG_NOTICE, "Channel %s has target %s\n", trunk_ref->chan->name, trunk_ref->chan->target->name);
			}
			target = trunk_ref->chan->target;
			target->releaseSLATrunk = releaseSLATrunk;
			trunk_ref->trunk->transfer_chan = target;
		}	
	}
	//Only set to NULL if it's the same as before conf_run
	struct ast_channel *station_dial_channel = NULL;
	if(original_trunk_ref_chan && original_trunk_ref_chan != trunk_ref->chan)
	{
		station_dial_channel = original_trunk_ref_chan;
	}
	else
	{
		station_dial_channel = trunk_ref->chan;
		trunk_ref->chan = NULL;
	}
	//End
	if (ast_atomic_dec_and_test((int *) &trunk_ref->trunk->active_stations) &&
		trunk_ref->state != SLA_TRUNK_STATE_ONHOLD_BYME) {
		ast_debug(1, "Number of active stations %d\n", trunk_ref->trunk->active_stations);
		ast_str_append(&conf_name, 0, ",K");
		admin_exec(NULL, ast_str_buffer(conf_name));
		trunk_ref->trunk->hold_stations = 0;
		//YZhou
		//If this flag is set, means transfer happened and the trunk is still in use
		if(!masqueraded)
		{
			ast_log(LOG_NOTICE, "Change trunk idle \n");
			sla_change_trunk_state(trunk_ref->trunk, SLA_TRUNK_STATE_IDLE, ALL_TRUNK_REFS, NULL);
		}
	}

	//YZhou
	int done = 0, saved_dial_count;
	if(station_dial_channel)
	{
		ast_log(LOG_NOTICE, "Find dial channel using channel %s \n", station_dial_channel->name);

		for(saved_dial_count = 0; saved_dial_count < 3; saved_dial_count++)
		{
			if(station->saved_dial[saved_dial_count])
			{
				if(ast_find_dial_channel_with_channel(station->saved_dial[saved_dial_count], station_dial_channel))
				{
 					ast_dial_join(station->saved_dial[saved_dial_count]);
                			ast_dial_destroy(station->saved_dial[saved_dial_count]);
                			station->saved_dial[saved_dial_count] = NULL;
					done = 1;
					break;
				}
			}
...
static void sla_handle_dial_state_event(void)
{
	...
	//YZhou
	int    answered = 0;
	struct sla_trunk_ref *answered_trunk_ref = NULL;
	...
	AST_LIST_TRAVERSE_SAFE_BEGIN(&sla.ringing_stations, ringing_station, entry) {
		...
		case AST_DIAL_RESULT_ANSWERED:
			...
			//YZhou
			answered = 1;
			answered_trunk_ref = s_trunk_ref;
			break;
	...
	if(answered) {
		...
		AST_LIST_TRAVERSE_SAFE_BEGIN(&sla.ringing_stations, ringing_station, entry) {
			AST_LIST_TRAVERSE(&ringing_station->station->trunks, trunk_ref, entry) {		
				if(trunk_ref->trunk == answered_trunk_ref->trunk) {
					ringing_station->answered_elsewhere = 1;
 					break;			
				}
			}
		}
...
static int sla_ring_station(struct sla_ringing_trunk *ringing_trunk, struct sla_station *station)
{
	...
	if(station->dial)	
	{
		...
		for (saved_dial_count = 0; saved_dial_count < 3; saved_dial_count++)
		{
			if(station->saved_dial[saved_dial_count] == NULL)
			{
				station->saved_dial[saved_dial_count] = station->dial;
				done = 1;
				break;
			}
		}
		if(!done)
		{
			ast_log(LOG_WARNING, "Too many station saved dials!!!\n");
		}
	}
	station->dial = dial;
...
static void sla_ring_stations(void)
{
	...
	AST_LIST_TRAVERSE(&sla.ringing_trunks, ringing_trunk, entry) {
		AST_LIST_TRAVERSE(&ringing_trunk->trunk->stations, station_ref, entry) {
			...
			//YZhou we want to ring the inuse station
...
static void sla_hangup_stations(void)
{
	...
	AST_LIST_TRAVERSE_SAFE_BEGIN(&sla.ringing_stations, ringing_station, entry) {
		...
		if (!trunk_ref) {
			AST_LIST_REMOVE_CURRENT(entry);
			//YZhou
			ast_dial_join_full(ringing_station->station->dial,ringing_station->answered_elsewhere);
			ast_dial_destroy(ringing_station->station->dial);
			ringing_station->station->dial = NULL;
			ast_free(ringing_station);
		}
...
//YZhou
//Created to handle per call barge control
static void sla_handle_toggle_per_call_barge_event(struct sla_event *event)
{
	ast_log(LOG_NOTICE, "Toggle barge for the call \n");
	if(event->trunk_ref->trunk->barge_disabled_per_call)
	{
		ast_log(LOG_NOTICE, "Barge is enabled for the call \n");
		event->trunk_ref->trunk->barge_disabled_per_call = 0;
	}
	else
	{
		ast_log(LOG_NOTICE, "Barge is disabled for the call \n");
		event->trunk_ref->trunk->barge_disabled_per_call = 1;
	}
}
//End

static void sla_handle_hold_event(struct sla_event *event, int savant)
{
	ast_log(LOG_NOTICE, "SLA handle hold event\n");
	...
	ast_log(LOG_NOTICE, "Trunk active stations %d hold stations %d\n", event->trunk_ref->trunk->active_stations, event->trunk_ref->trunk->hold_stations);
	if (event->trunk_ref->trunk->active_stations == 1) {
		...
		if(event->trunk_ref->chan && event->trunk_ref->chan->name)
		{
			ast_log(LOG_NOTICE, "Channel init hold: %s \n", event->trunk_ref->chan->name);
		}
		event->trunk_ref->trunk->on_hold = 1;
		event->trunk_ref->trunk->chan_init_hold = event->trunk_ref->chan;
		ast_indicate(event->trunk_ref->trunk->chan, AST_CONTROL_HOLD);
	}
	//YZhou
	//Only hang up if there is still other stations active on the trunk
	if(event->trunk_ref->trunk->active_stations > 1 || savant)
	{
...
//YZhou
//Created to handle unhold event
static void sla_handle_unhold_event(struct sla_event *event)
{
	ast_log(LOG_NOTICE, "SLA handle unhold event\n");
	ast_atomic_fetchadd_int((int *) &event->trunk_ref->trunk->hold_stations, -1);
	event->trunk_ref->state = SLA_TRUNK_STATE_UP;
	sla_change_trunk_state(event->trunk_ref->trunk, SLA_TRUNK_STATE_UP,
                ALL_TRUNK_REFS, NULL);
	event->trunk_ref->trunk->on_hold = 0;
	if(event->trunk_ref->trunk->chan)
	{
		ast_indicate(event->trunk_ref->trunk->chan, AST_CONTROL_UNHOLD);
	}
	event->trunk_ref->trunk->chan_init_hold = NULL;
}
//End
...
static void *sla_thread(void *data)
{
	...
	while (!sla.stop) {
		...
		while ((event = AST_LIST_REMOVE_HEAD(&sla.event_q, entry))) {
			...
			switch (event->type) {
			case SLA_EVENT_HOLD:
				sla_handle_hold_event(event, 0);
				break;
			//YZhou
			case SLA_EVENT_HOLD_SAVANT:
                                sla_handle_hold_event(event, 1);
                                break;
			case SLA_EVENT_UNHOLD:
				sla_handle_unhold_event(event);
				break;
			case SLA_EVENT_TOGGLE_PER_CALL_BARGE:
				sla_handle_toggle_per_call_barge_event(event);
				break;
			//End
...
static void *dial_trunk(void *data)
{
	...
	for (;;) {
		...
		case AST_DIAL_RESULT_TIMEOUT:
		case AST_DIAL_RESULT_UNANSWERED:
			ast_indicate(args->trunk_ref->chan, -1);
			done = 1;
			break;
		case AST_DIAL_RESULT_TRYING:
			break;
		...
		case AST_DIAL_RESULT_RINGING:
			if (!is_ringing)
				ast_indicate(args->trunk_ref->chan, AST_CONTROL_RINGING); 
			is_ringing = 1;
			break;
		case -1:
			ast_indicate(args->trunk_ref->chan, -1);
		...
		if (done)
			break;
        //YZhou port asterisk 20462 patch
        /* check that SLA station that originated trunk call is still alive */
        /* Forward call from VM does not work with this fix, so comment out.
	...
	ast_mutex_unlock(args->cond_lock);
	//YZhou
	//Store the name of channel that dial the trunk 
	if(trunk_ref->chan->name)
	{
		ast_copy_string(trunk_ref->trunk->dial_chan_name, trunk_ref->chan->name, sizeof(trunk_ref->trunk->dial_chan_name));
		ast_log(LOG_NOTICE, "Dial trunk channel name %s\n", trunk_ref->trunk->dial_chan_name);
	}
	//End
	if (conf) {
	...
	//YZhou
	int masqueraded = 0;
	if(trunk_ref->trunk && trunk_ref->trunk->chan)
	{
		masqueraded = trunk_ref->trunk->chan->masqueraded;
		ast_log(LOG_NOTICE, "Dial trunk channel after conf_run %s masqueraded %d\n", trunk_ref->trunk->chan->name, trunk_ref->trunk->chan->masqueraded);
	}
	...
	//YZhou
	//But if it's masqueraded due to transfer, the trunk is still in use
	else
	{
		struct sla_trunk *transfer_trunk;
		AST_RWLIST_RDLOCK(&sla_trunks);
		//Go through all trunks
        AST_RWLIST_TRAVERSE(&sla_trunks, transfer_trunk, entry) {
            if(transfer_trunk->name)
			{
                ast_debug(1, "Trunk name %s\n", transfer_trunk->name);
			}
			//Got the name of dial out channel
            if(transfer_trunk->dial_chan_name[0])
            {
                ast_debug(1, "Trunk dial channel%s\n", transfer_trunk->dial_chan_name);
				/* If the name of dial out channel for a trunk is subset of channel name for this trunk, 
                    means the call was transferred to that trunk. When that trunk is released, we need to
                    change state for this trunk, too.
				*/
				if(trunk_ref->trunk && trunk_ref->trunk->chan && trunk_ref->trunk->chan->name)
				{
                    if(strstr(trunk_ref->trunk->chan->name, transfer_trunk->dial_chan_name))
                    {
                        ast_log(LOG_NOTICE, "Bind trunk %s to trunk %s\n", trunk_ref->trunk->name, transfer_trunk->name);
                        transfer_trunk->transfer_trunk = trunk_ref->trunk;
						//Since the call is transferred, join is not allowed.
                        transfer_trunk->barge_disabled_per_call = 1;
						break;
                    }
                }
            }
...
static int sla_station_exec(struct ast_channel *chan, const char *data)
{
	...
	//YZhou
	ast_debug(1, "Enter function sla_station_exec\n");
	if (ast_strlen_zero(data)) {
	...
	if (!ast_strlen_zero(trunk_name)) {
		//YZhou
		ast_log(LOG_NOTICE, "Find trunk ref by name %s\n", trunk_name);
	...
	//YZhou
	/* Check if there is active transfer call from this trunk to another trunk. 
           If there is, this trunk cannot be used, act on the other trunk.
	*/
	if(trunk_ref)
	{
		struct sla_trunk *tmp_trunk;
		AST_RWLIST_RDLOCK(&sla_trunks);
		AST_LIST_TRAVERSE(&sla_trunks, tmp_trunk, entry) {
			if (tmp_trunk->transfer_trunk == trunk_ref->trunk)
			{
				if(trunk_ref->trunk->name && tmp_trunk->name)
				{
					ast_log(LOG_NOTICE, "Trunk %s has been transferred to trunk %s\n", trunk_ref->trunk->name, tmp_trunk->name);
				}
				trunk_ref = sla_find_trunk_ref_byname(station, tmp_trunk->name);
				break;
			}
		}
		AST_RWLIST_UNLOCK(&sla_trunks);
	...
	//YZhou
	ast_log(LOG_NOTICE, "Trunk state %d active stations %d\n", trunk_ref->state, trunk_ref->trunk->active_stations);
	if (trunk_ref->state == SLA_TRUNK_STATE_ONHOLD_BYME) {
	...
	//YZhou
	//If trunk was put on hold with one station, get the channel that initiated hold 
	struct ast_channel *hold_channel = NULL;
	if(trunk_ref->trunk->active_stations == 1 && trunk_ref->trunk->on_hold)
	{
		if(trunk_ref->trunk->chan_init_hold)
		{
			if(trunk_ref->trunk->chan_init_hold->name)
			{
				ast_log(LOG_NOTICE, "Hold channel %s\n", trunk_ref->trunk->chan_init_hold->name);
			}
			hold_channel = trunk_ref->trunk->chan_init_hold;
			trunk_ref->trunk->chan_init_hold = NULL;
		}				
	}
	//End
	ast_log(LOG_NOTICE, "Set trunk_ref->chan %s\n", chan->name);
	trunk_ref->chan = chan;
	...
	//YZhou
	ast_log(LOG_NOTICE, "Trunk on_hold %d active stations %d\n", trunk_ref->trunk->on_hold, trunk_ref->trunk->active_stations);
	//if (ast_atomic_fetchadd_int((int *) &trunk_ref->trunk->active_stations, 1) == 0 &&
	//	trunk_ref->trunk->on_hold) {
	ast_atomic_fetchadd_int((int *) &trunk_ref->trunk->active_stations, 1);
	if(trunk_ref->trunk->on_hold) {
		trunk_ref->trunk->on_hold = 0;
		ast_indicate(trunk_ref->trunk->chan, AST_CONTROL_UNHOLD);
		sla_change_trunk_state(trunk_ref->trunk, SLA_TRUNK_STATE_UP, ALL_TRUNK_REFS, NULL);
		//YZhou
		/* Someone join the call, hangup the one initiated the hold. Remember, the one who can initiate
		   the hold on the trunk is the last active station on the trunk, and is not released when it starts 
                   hold. This is the case someone put call on hold from one station, then pick up from another station.
		*/
		if(hold_channel)
		{
			ast_log(LOG_NOTICE, "Hang up original hold channel %s\n", hold_channel->name);
			ast_softhangup(hold_channel, AST_SOFTHANGUP_DEV);
		}
		//End
	}
 	...
 	if(trunk_ref && trunk_ref->chan)
	{
		...
		if(trunk_ref->chan->target)
		{
			if(trunk_ref->chan->target->name)
			{
				ast_log(LOG_NOTICE, "Channel %s has target channel %s \n", trunk_ref->chan->name, trunk_ref->chan->target->name);
			}
			target = trunk_ref->chan->target;
			target->releaseSLATrunk = releaseSLATrunk;
			trunk_ref->trunk->transfer_chan = target;
	...
	{
		ast_debug(1, "No trunk ref channel\n");
	}
	//YZhou only set to NULL when it's the same as before conf_run
	if(chan && trunk_ref->chan != chan)
	{
	}
	else
	{
		ast_log(LOG_NOTICE, "Reset trunk_ref channel to NULL\n");
		trunk_ref->chan = NULL;
	}
	//End
	ast_debug(1, "Active stations before dec_and_test %d\n", trunk_ref->trunk->active_stations);
	if (ast_atomic_dec_and_test((int *) &trunk_ref->trunk->active_stations) &&
		trunk_ref->state != SLA_TRUNK_STATE_ONHOLD_BYME) {
...
static int sla_trunk_exec(struct ast_channel *chan, const char *data)
{
	...
	if(trunk->chan && trunk->chan->name)
	{
		ast_log(LOG_NOTICE, "Trunk channel before conf_run %s\n", trunk->chan->name);
	}
	conf_run(chan, conf, &conf_flags, opts);
	//YZhou
	/* If trunk channel is masqueraded due to transfer, find the trunk that the call transferred to, and
	   bind two trunks
	*/
	int masqueraded = 0;
	if(trunk && trunk->chan)
	{
		if(trunk->chan->name)
		{
			ast_log(LOG_NOTICE, "Trunk channel after conf_run %s masqueraded %d\n", trunk->chan->name, trunk->chan->masqueraded);
		}
		masqueraded = trunk->chan->masqueraded;
	}
	if(masqueraded)
	{
		struct sla_trunk *transfer_trunk;
		...
		AST_RWLIST_TRAVERSE(&sla_trunks, transfer_trunk, entry) {
			...
			{
				ast_debug(1, "Trunk dial channel%s\n", transfer_trunk->dial_chan_name);
				if(trunk && trunk->chan && trunk->chan->name)
				{
					if(strstr(trunk->chan->name, transfer_trunk->dial_chan_name))
					{
						if(trunk->name && transfer_trunk->name)
						{
							ast_log(LOG_NOTICE, "Bind trunk %s to trunk %s\n", trunk->name, transfer_trunk->name);
						}
						transfer_trunk->transfer_trunk = trunk;
						transfer_trunk->barge_disabled_per_call = 1;
						break;
					}
				}
...
static int sla_build_trunk(struct ast_config *cfg, const char *cat)
{
	...
	trunk->barge_disabled_per_call = 0;
	trunk->transfer_trunk = NULL;
	trunk->chan_init_hold = NULL;
	trunk->dial_chan_name[0] = '\0';
	trunk->transfer_chan  = NULL;
	//End
...
static int sla_build_station(struct ast_config *cfg, const char *cat)
{
	...
	//YZhou
	station->saved_dial[0] = NULL;
	station->saved_dial[1] = NULL;
	station->saved_dial[2] = NULL;
	for (var = ast_variable_browse(cfg, cat); var; var = var->next) {
...
static int sla_togglebarge_exec(struct ast_channel *chan, const char *data)
{
	ast_indicate(chan, AST_CONTROL_TOGGLE_PER_CALL_BARGE);
	return 0;
}
...
static int unload_module(void)
{
	...
	res |= ast_unregister_application(slatogglebarge_app);
...
static int load_module(void)
{
	...
	res |= ast_register_application_xml(slatogglebarge_app, sla_togglebarge_exec);
