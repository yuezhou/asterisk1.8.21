...
static void mixmonitor_ds_close_fs(struct mixmonitor_ds *mixmonitor_ds)
{
	if (mixmonitor_ds->fs) {
		...
		if (mixmonitor_ds->txt) {
			fclose(mixmonitor_ds->txt);
			mixmonitor_ds->txt = NULL;
		}
...
static void *mixmonitor_thread(void *obj) 
{
	...
	txt = &mixmonitor->mixmonitor_ds->txt;
	...
	while (mixmonitor->audiohook.status == AST_AUDIOHOOK_STATUS_RUNNING && !mixmonitor->mixmonitor_ds->fs_quit) {
		...
		if (!ast_test_flag(mixmonitor, MUXFLAG_BRIDGED) || (mixmonitor->autochan->chan && ast_bridged_channel(mixmonitor->autochan->chan))) {
			ast_mutex_lock(&mixmonitor->mixmonitor_ds->lock);
			int new_moh_flag = ast_test_flag(mixmonitor->autochan->chan, AST_FLAG_MOH);
			struct ast_channel *peer = ast_bridged_channel(mixmonitor->autochan->chan);
			if (peer && ast_test_flag(peer, AST_FLAG_MOH)) {
				new_moh_flag = 1;
			}
			...
			if (!*fs && !errflag && !mixmonitor->mixmonitor_ds->fs_quit) {
				...
				char *txtfile = NULL;
				ast_asprintf(&txtfile, "%s.txt", mixmonitor->filename);
				*txt = fopen(txtfile, "a");
				if (!*txt) {
			...
			if (new_moh_flag) {
				if (!moh) {
					moh_start = ast_tellstream(*fs);
					moh = 1;
				}
			} else {
				if (moh) {
					char data[80];
					int data_size = snprintf(data, sizeof(data), "%u,%u\n", (unsigned int)moh_start, (unsigned int)ast_tellstream(*fs));
					fwrite(data, data_size, 1, *txt);
					moh = 0;
				}
			}
