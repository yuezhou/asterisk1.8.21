...
#include "sit_detect/sit_detect.h"
		...
		<syntax>
			...
			<parameter name="options" required="false">
			   <optionlist>
				...
				<option name="E">
					<argument name="x" required="true" />
					<para>Allow the calling party to trigger an emergency call.
					The call will first attempt to go out the channel(s) specified.  If none 
					are avaiable, it will walk the list for the first channel specified with
					a prioirty lower than <replaceable>x</replaceable>.</para>
				</option>
				<option name="f">
...
#define OPT_SIT_DETECT       (1LLU << 41)
#define OPT_EMERGENCY        (1LLU << 42)
...
AST_APP_OPTIONS(dial_exec_options, BEGIN_OPTIONS
	...
	AST_APP_OPTION('B', OPT_SIT_DETECT),
	AST_APP_OPTION('C', OPT_RESETCDR),
	AST_APP_OPTION('c', OPT_CANCEL_ELSEWHERE),
	AST_APP_OPTION('d', OPT_DTMF_EXIT),
	AST_APP_OPTION_ARG('D', OPT_SENDDTMF, OPT_ARG_SENDDTMF),
	AST_APP_OPTION('e', OPT_PEER_H),
	AST_APP_OPTION_ARG('E', OPT_EMERGENCY, OPT_ARG_EMERGENCY),
...
static struct ast_channel *wait_for_answer(struct ast_channel *in,
	...
	const int ignore_cc,
	struct ast_party_id *forced_clid, struct ast_party_id *stored_clid, int sit_tone_detect)
{
	struct chanlist *o;
	...
	struct sit_detector *sit_det = NULL;
	...
	if (sit_tone_detect) {
		sit_det = sit_detector_new();

		for (o = outgoing; o; o = o->next) {
			int res;
			res = ast_set_read_format(o->chan, AST_FORMAT_SLINEAR);
			if (res < 0) {
				ast_log(LOG_WARNING, "SIT DETECTOR: Failed to set channel %s to slinear, turning off SIT Detection all together\n", o->chan->name);
				sit_detector_free(sit_det);
				sit_det = NULL;
				sit_tone_detect = 0;
			}
		}
	...
	while ((*to = ast_remaining_ms(start, orig)) && !peer) {
		...
		for (o = outgoing; o; o = o->next) {
			...
			case AST_FRAME_VOICE:
				if (sit_det) {
					int sit_cause;
					sit_detector_process_frame(sit_det, f->data.ptr, f->samples);
					if ((sit_cause = sit_detector_detected_tones(sit_det)) != SIT_NONE) {
						char * code;
						ast_verb(3, "SIT DETECTED: %s\n", sit_detector_get_cause(sit_cause));
						...
						pbx_builtin_setvar_helper(in, "DIALSITDETECTED", code);
						ast_clear_flag64(o, DIAL_STILLGOING);
						handle_cause(AST_CAUSE_CONGESTION, &num);
					}
				}
...
static int dial_exec_full(struct ast_channel *chan, const char *data, struct ast_flags64 *peerflags, int *continue_exec)
{
	...
	if (ast_test_flag64(&opts, OPT_SIT_DETECT)) {
		sit_tone_detect = 1;
	}
	...
	while ((cur = strsep(&rest, "&")) ) {
		...
		if (ast_test_flag64(&opts, OPT_EMERGENCY) && !ast_strlen_zero(opt_args[OPT_ARG_EMERGENCY])) {
			if ((!strcasecmp(tech, "zap") || !strcasecmp(tech, "dahdi")) && (atoi(opt_args[OPT_ARG_EMERGENCY]) > 0)) {
				char *t = emergency_numsubst;
				size_t t_size = sizeof(emergency_numsubst);
				ast_build_string(&t, &t_size, "e%s/%s", opt_args[OPT_ARG_EMERGENCY], number);
			}
			else {
				if (!strcasecmp(tech, "zap") || !strcasecmp(tech, "dahdi"))
					ast_log(LOG_WARNING, "Dial flag 'E' not allowed for channel type %s\n", tech);
				if (atoi(opt_args[OPT_ARG_EMERGENCY]) > 0)
					ast_log(LOG_WARNING, "Arguments to Dial flag 'E' must be an integer\n");
			}
		}
		...
		tc = ast_request(tech, chan->nativeformats, chan, !ast_strlen_zero(emergency_numsubst)?emergency_numsubst:numsubst, &cause);
		if (!tc) {
	...
	peer = wait_for_answer(chan, outgoing, &to, peerflags, opt_args, &pa, &num, &result,
		dtmf_progress, ignore_cc, &forced_clid, &stored_clid, sit_tone_detect);
