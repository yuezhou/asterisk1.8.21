...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 372390 $")
...
struct number {
	...
	char options[50];       /*!< Options that apply per number line */
...
static struct number *create_followme_number(const char *number, int timeout, int numorder, const char *options)
{
	struct number *cur;
	...
	if (options)
		ast_copy_string(cur->options, options, sizeof(cur->options));
	else
		cur->options[0] = '\0';
	...
	while ((cat = ast_category_browse(cfg, cat))) {
		...
		while (var) {
			if (!strcasecmp(var->name, "number")) {
				...
				cur = create_followme_number(numberstr, timeout, numorder, options);
				if (cur) {
...
static int app_exec(struct ast_channel *chan, const char *data)
{
	...
	AST_LIST_TRAVERSE(&f->numbers, nm, entry) {
		newnm = create_followme_number(nm->number, nm->timeout, nm->order, nm->options);
		if (newnm) {
	...
	if (targs.status != 100) {
		...
		pbx_builtin_setvar_helper(chan, "DIALSTATUS", "ANSWER");
