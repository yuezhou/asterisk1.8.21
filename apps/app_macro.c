...
static void macro_fixup(void *data, struct ast_channel *old_chan, struct ast_channel *new_chan)
{
	...
	int i;
	char varname[10];
	pbx_builtin_setvar_helper(old_chan, "MACRO_DEPTH", "0");
	pbx_builtin_setvar_helper(old_chan, "MACRO_CONTEXT", NULL);
	pbx_builtin_setvar_helper(old_chan, "MACRO_EXTEN", NULL);
	pbx_builtin_setvar_helper(old_chan, "MACRO_PRIORITY", NULL);
	pbx_builtin_setvar_helper(old_chan, "MACRO_OFFSET", NULL);
	for (i = 1; i < 100; i++) {
		snprintf(varname, sizeof(varname), "ARG%d", i);
		while (pbx_builtin_getvar_helper(old_chan, varname)) {
			/* Kill all levels of arguments */
			pbx_builtin_setvar_helper(old_chan, varname, NULL);
		}
	}
#endif
}
