...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 384086 $")
...
struct hook_flash {
	...
	int  control_sent;
	struct timeval tv;
	AST_LIST_ENTRY(hook_flash) entry;
};

AST_LIST_HEAD_NOLOCK(hook_flash_list, hook_flash);

struct hook_flash_list flash_candidates = {NULL, NULL};
...
//YZhou
static int send_hook_flash(char *sender,  char digit, int begin);
...
static struct ast_frame *create_dtmf_frame(struct ast_rtp_instance *instance, enum ast_frame_type type, int compensate)
{
	...
	//YZhou
	//Create a control frame to disable barge on SLA line
 	} else if (rtp->resp == 'Z') {
		if(type == AST_FRAME_DTMF_BEGIN)
		{
                	rtp->f.frametype = AST_FRAME_CONTROL;
                	rtp->f.subclass.integer = AST_CONTROL_TOGGLE_PER_CALL_BARGE;
		}
		else
		{
			rtp->resp = 0;
	...
	if (ast_rtp_instance_get_prop(instance, AST_RTP_PROPERTY_DTMF_COMPENSATE)) {
		...
		if (event_end & 0x80) {
			...
			if (rtp->resp) {
				...
				//YZhou	
				//Check DTMF sequence			
				int dtmfSequence = send_hook_flash(ast_sockaddr_stringify(&remote_address), resp, 0);
				//Hook flask
				if(dtmfSequence == 1)
				{
					rtp->resp = '!';
				}
				//Toggle enable/disable per call barge on SLA line
				else if(dtmfSequence == 2)
				{
					rtp->resp = 0;
					rtp->dtmf_duration = rtp->dtmf_timeout = 0;
				}
			} else {
				//YZhou check if it matches ** pattern indicating hook flash				
				int dtmfSequence = send_hook_flash(ast_sockaddr_stringify(&remote_address), resp, 1);
				if(dtmfSequence == 1)
				{
					resp = '!';
				}
...
static int send_hook_flash(char *sender, char digit, int begin)
{
	int found = 0, rtn = 0;
	struct timeval tv;
	struct hook_flash   *candidate = NULL;

	tv = ast_tvnow();

 	AST_LIST_TRAVERSE(&flash_candidates, candidate, entry) {
		//The sender is in record, means it sent '*' before			
		if (!strcmp(candidate->sender, sender)) {
			found = 1;
			if(begin == 1)
			{
				//New digit is not '*' or '#', remove entry
				if(digit != '*' && digit != '#')
				{
					ast_log(LOG_DEBUG, "Not ** or *#, remove candidate\n");
					AST_LIST_REMOVE(&flash_candidates, candidate, entry);
					ast_free(candidate);
					rtn = 0;
				}
				//Match '**' or '*#' sequence, check time
				else
				{
					if(digit == '*')
					{
						//Time too long. Treat the second '*' as the new candidate
						if(ast_tvdiff_ms(tv, candidate->tv) > 2000)
						{	
							ast_log(LOG_DEBUG, "The time is longer than 2 seconds between **, not flash\n");
							candidate->tv    = tv;
							candidate->flash = 0;
							candidate->control_sent = 0;
							rtn = 0;
						}
						else
						{	
							//To avoid flash back if there is one extra '*' by mistake
							if(candidate->flash == 1)
							{
								ast_log(LOG_DEBUG, "Extra *, treat as first for next potential flash\n");
								candidate->tv    = tv;
								candidate->flash = 0;
								rtn = 0;
							}
							...
							{
								ast_log(LOG_DEBUG, "Meet flash condition, send flash\n");						
								rtn = 1;
								candidate->flash = 1;
							}
						}
					...
					{
						//Time too long, remove candidate
						if(ast_tvdiff_ms(tv, candidate->tv) > 2000)
						{
							ast_log(LOG_DEBUG, "The time is longer than 2 seconds between *#, not barge off command\n");
							AST_LIST_REMOVE(&flash_candidates, candidate, entry);
							ast_free(candidate);
							rtn = 0;
						}
						...
						{
							candidate->flash = 2;
							if(!candidate->control_sent)
							{
								ast_log(LOG_DEBUG, "Meet barge off condition, send control\n"); 
								candidate->control_sent = 1;
								rtn = 2;
							}
							else
							{
			...
			{
				if(candidate->flash && !candidate->control_sent)
				{
					rtn = candidate->flash;
				}
				else
	...
	if(!found)
	{
		...
		{		
			...
			{
				ast_log(LOG_DEBUG, "First *, create flash candidate\n");
				strcpy(candidate->sender, sender);
				candidate->tv           = ast_tvnow();
				candidate->flash        = 0;
				candidate->control_sent = 0;			
				AST_LIST_INSERT_HEAD(&flash_candidates, candidate, entry);			
			}
		}	
	}
	return rtn;					
