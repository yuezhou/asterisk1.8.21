...
static int pbx_builtin_exportvar(struct ast_channel *, const char *);
...
} builtins[] =
{
	...
	{ "ExportVar",	    pbx_builtin_exportvar },
...
int pbx_builtin_exportvar(struct ast_channel *chan, const char *data)
{
	char *name;
	char *value;
	char *channel;
	...
	if (channel && value && name) { /*! \todo XXX should do !ast_strlen_zero(..) of the args ? */
		...
		if (chan2) {
			...
			if (name[strlen(name)-1] == ')') {
				char *function = ast_strdupa(name);
				ast_func_write(chan2, function, value);
			} else {
				pbx_builtin_setvar_helper(chan2, name, S_OR(value, ""));
			}
