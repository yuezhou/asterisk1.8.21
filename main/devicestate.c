...
/*! \brief A list of devices to ask about again */
struct devstate_no_provider {
	char provider[40];
	char data[256];
	AST_RWLIST_ENTRY(devstate_no_provider) list;
};

/*! \brief A list of devices without providers that we'll hold our breath for */
static AST_RWLIST_HEAD_STATIC(devstate_no_providers, devstate_no_provider);
...
int ast_devstate_prov_add(const char *label, ast_devstate_prov_cb_type callback)
{
	...
	struct devstate_no_provider *noprov;
	...
	AST_RWLIST_WRLOCK(&devstate_no_providers);
	AST_RWLIST_TRAVERSE_SAFE_BEGIN(&devstate_no_providers, noprov, list) {
		if (!strcasecmp(noprov->provider, label)) {
			AST_RWLIST_REMOVE_CURRENT(list);
			ast_device_state_changed("%s:%s", noprov->provider, noprov->data);
			free(noprov);
			break;
		}
	}
	AST_RWLIST_TRAVERSE_SAFE_END;
	AST_RWLIST_UNLOCK(&devstate_no_providers);
...
static int getproviderstate(const char *provider, const char *address)
{
	...
	if (!found) {
		struct devstate_no_provider *noprov = ast_calloc(1, sizeof(*noprov));
	
		if (noprov) {
			ast_copy_string(noprov->provider, provider, sizeof(noprov->provider));
			ast_copy_string(noprov->data, address, sizeof(noprov->data));

			AST_RWLIST_WRLOCK(&devstate_no_providers);
			AST_RWLIST_INSERT_HEAD(&devstate_no_providers, noprov, list);
			AST_RWLIST_UNLOCK(&devstate_no_providers);
		}
	}
	return res;
