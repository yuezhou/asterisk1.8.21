...
static struct ast_module *load_dynamic_module(const char *resource_in, unsigned int global_symbols_only, struct ast_heap *resource_heap)
{
	...
	*/
	...
	/* this prevents lumenvox module from hanging */
	if (!strcmp(resource_being_loaded->resource, "res_speech_lumenvox.so"))
	{
		ast_log(LOG_NOTICE, "Sleeping for 2 seconds before loading module %s\n", resource_being_loaded->resource);
		sleep(2);
	}
