...
static int begin_dial_channel_p1(struct ast_dial_channel *channel, struct ast_channel *chan)
{
...
static int begin_dial_channel_p2(struct ast_dial_channel *channel, struct ast_channel *chan)
{
	int res = 1;
	char numsubst[AST_MAX_EXTENSION];
...
{
	int ret;
	ret = begin_dial_channel_p1(channel, chan);
	if (ret)
		return ret;
	return begin_dial_channel_p2(channel, chan);
}
...
static int handle_call_forward(struct ast_dial *dial, struct ast_dial_channel *channel, struct ast_channel *chan)
{
	...
	begin_dial_channel_p1(channel, chan);

	/* XXX */
	struct ast_party_redirecting redirecting;
	ast_channel_set_redirecting(channel->owner, &original->redirecting, NULL);

	ast_party_redirecting_init(&redirecting);
	ast_party_redirecting_copy(&redirecting, &channel->owner->redirecting);
	if (ast_channel_redirecting_macro(NULL, channel->owner, &redirecting, 0, 0)) {
	...
	begin_dial_channel_p2(channel, chan);
...
/*! YZhou */
int ast_find_dial_channel_with_channel(struct ast_dial *dial, struct ast_channel *owner)
{
        struct ast_dial_channel *channel = NULL;
	int found = 0;
...
{
	return ast_dial_join_full(dial,0);
}
enum ast_dial_result ast_dial_join_full(struct ast_dial *dial, int complete_elsewhere)
{
	pthread_t thread;
		...
		if (chan) {
			...
			if(complete_elsewhere){
				ast_set_flag(chan, AST_FLAG_ANSWERED_ELSEWHERE);
                        	chan->hangupcause = AST_CAUSE_ANSWERED_ELSEWHERE;
			}
			/* Stop the thread */
